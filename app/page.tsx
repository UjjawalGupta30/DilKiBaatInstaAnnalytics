...
export default function Home() {
  return (
    <main className="shell">
      ...
      <section className="drafts">{drafts.map((draft, i) => <article className="draft" key={draft.id}><div className={`cover cover${i+1}`}><span>{draft.format}</span><b>{i === 0 ? <>Some people<br />stay with us.</> : i === 1 ? <>When friendship<br />quietly changes.</> : <>You don't owe<br />everyone an explanation.</>}</b><small>DIL KI BAAT</small></div><div className="draftbody"><div className="draftmeta"><span>{draft.id}</span><span className={`status ${draft.status==="Approved"?"approved":""}`}>{draft.status}</span></div><h3>{draft.title}</h3><p className="hook">“{draft.hook}”</p><div className="draftactions"><button type="button" className="secondary small">Preview</button><button type="button" className="primary small">{draft.status==="Approved"?"View approval":"Review draft"}</button></div></div></article>)}</section>
